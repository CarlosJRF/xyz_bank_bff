...
public class BffWebService {
    ...
    public CuentaWebDetalleDTO obtenerDetalleWeb(Long cuentaId) {
        ...
        List movimientos = agregado.transacciones().stream()
            .map(t -> new MovimientoWebDTO(t.fecha(), t.tipoTransaccion(), t.monto(), t.descripcion()))
            .toList();

        BigDecimal ingresos = movimientos.stream().filter(m -> m.monto().compareTo(BigDecimal.ZERO) > 0).map(MovimientoWebDTO::monto).reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal egresos = movimientos.stream().filter(m -> m.monto().compareTo(BigDecimal.ZERO) < 0).map(MovimientoWebDTO::monto).reduce(BigDecimal.ZERO, BigDecimal::add);
